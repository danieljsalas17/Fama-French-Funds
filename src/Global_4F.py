...
In Terminal, change to the appropriate directory and enter:
    ...
    $ python3 Global_4F.py

To run quietly in terminal, but save to log file:

    $ python3 Global_4F.py > logs/Global_4F_log.txt

To run loud in both:

    $ python3 Global_4F.py | tee logs/Global_Ff_log.txt
...
if __name__=="__main__":
    ...
    factors_path = path_stem + 'data/global_4factors.csv'
    min_obs = 120
    sim_cutoff = 15
    n_simulations = 1000
    ...
    # Save plots with just best and worst for front page README.md
    ...
    plt.savefig(path_stem+'charts/Global-4factor-plots.png')

    # Save plots with 3 best and 3 worst
    fig, axes = multi_plot(plot_type=['cdf','kde','hist'],
                           statistic=['alpha','t-stat'],
                           betas=B,tstats=B/BSE,
                           alphas_sim=ALPH,tstats_sim=ALPH/A_SE,
    ...
    fig.suptitle("Injected Standard Deviation of Alpha = {:.2f}".format(0.))
    fig.tight_layout(rect=[0, 0.03, 1, 0.95])
    plt.savefig(path_stem+'charts/Global/4factor-plots-{}.png'.format(0))
    plt.clf()
    plt.close('all')
    #---------------------------------------------------------------------------
    ...
    for i,stdev in enumerate(std_range):
        ...
        fig.tight_layout(rect=[0, 0.03, 1, 0.95])
        plt.savefig(path_stem+'charts/Global/4factor-plots-{}.png'.format(i+1))
        plt.clf()
        plt.close('all')
        print("DONE!")
    #----------------------------------------------------------------------------
    ...
    data_a.to_csv(path_stem+'tables/Global-4factor-alphas.csv')
    data_t.to_csv(path_stem+'tables/Global-4factor-tstats.csv')
